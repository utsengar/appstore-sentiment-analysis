...
def getReviews(appStoreId, appId):
    ...
    for i in range(len(reviews)):
        output.append(
            {"review": reviews[i], "rank": stars[i], "topic": topics[i]})
    return output
...
def _print_reviews(reviews, country):
    ...
    '''
    if len(reviews) > 0:
        print "Reviews in %s:" % (country.encode('utf-8'))
        sumRank = 0
        for review in reviews:
            ...
            sumRank += review["rank"]
        print "Number of reviews in %s: %d, avg rank: %.2f\n" % (country.encode('utf-8'), len(reviews), 1.0 * sumRank / len(reviews))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='AppStoreReviewsScrapper command line.', epilog='To get your application Id look into the AppStore link to you app, for example http://itunes.apple.com/pl/app/autobuser-warszawa/id335042980?mt=8 - app Id is the number between "id" and "?mt=0"')
    parser.add_argument('-i', '--id', default=0, metavar='AppId',
                        type=int, help='Application Id (see below)')
    parser.add_argument(
        '-c', '--country', metavar='"Name"', type=str, default='all',
